...
if len(trades_df) > 0:
    ...
    for reason, count in sorted(skipped_by_reason.items()):
        print(f"    {reason:20s}: {count:4d} 个")

    print("\n" + "=" * 80)
    print("回测结果：6持仓（带股价限制）")
    print("=" * 80)
...
trades_df.to_csv('/home/tom/stock-verify/tdcc-whale-accumulation/backtest_6positions_with_price_filter_results.csv', index=False)
print(f"\n✓ 结果已保存")
