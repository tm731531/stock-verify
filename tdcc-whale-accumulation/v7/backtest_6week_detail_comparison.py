...
print("="*80)
print("V7 策略回測詳細對比：42天日曆 vs 90天日曆")
print("="*80)
...
def backtest(hold_days, hold_label):
    print(f"\n{'='*80}")
    print(f"回測：持倉期 {hold_label} ({hold_days} 天日曆)")
    print(f"{'='*80}\n")
    ...
    for current_idx, current_date in enumerate(all_dates):
        ...
        for stock_code, pos in list(positions.items()):
            ...
            if not should_exit and calendar_days >= hold_days:
                should_exit = True
            ...
            if should_exit:
                ...
                trades.append({
                    ...
                    'days_held': calendar_days, 'gross_return_pct': gross_ret,
    ...
    for stock_code, pos in positions.items():
        ...
        calendar_days = (last_date_obj - buy_date_obj).days
    ...
    if trades:
        ...
        # 分離已平倉和未平倉
        closed = trades_df[trades_df['exit_reason'] != 'data_end']
        ...
        # 詳細交易列表
        print(f"\n詳細交易 (按買入日期):")
        print(f"{'代碼':>6} {'引擎':>5} {'買日':>10} {'賣日':>10} {'買價':>8} {'賣價':>8} {'報酬':>7} {'天':>4} {'損益':>10} {'出場'}")
        print(f"{'-'*95}")
        for _, row in trades_df.sort_values('buy_date').iterrows():
            eng = 'M' if row['engine'] == 'main' else 'B'
            print(f"{row['stock']:>6} {eng:>5} {row['buy_date']:>10} {row['exit_date']:>10} "
                  f"{row['entry_price']:>8.1f} {row['exit_price']:>8.1f} {row['net_return_pct']:>+6.1f}% {row['days_held']:>3}d "
                  f"{row['pnl_twd']:>+10,.0f} {row['exit_reason']}")
...
# ── 執行回測 ──
result_42 = backtest(42, "42天（6週）")
result_90 = backtest(90, "90天")

# ── 對比 ──
print(f"\n{'='*80}")
print("對比結果：42天 vs 90天")
print(f"{'='*80}\n")

print(f"{'指標':<15} {'42天':>15} {'90天':>15} {'差異':>15}")
print(f"{'-'*60}")
print(f"{'年化報酬':<15} {result_42['annual_return']:>14.1f}% {result_90['annual_return']:>14.1f}% {result_42['annual_return']-result_90['annual_return']:>+14.1f}%")
print(f"{'總利潤':<15} {result_42['total_pnl']:>14,} {result_90['total_pnl']:>14,} {result_42['total_pnl']-result_90['total_pnl']:>+14,}")
print(f"{'交易筆數':<15} {result_42['trades']:>15} {result_90['trades']:>15} {result_42['trades']-result_90['trades']:>+15}")
print(f"{'勝率':<15} {result_42['win_rate']:>14.1f}% {result_90['win_rate']:>14.1f}% {result_42['win_rate']-result_90['win_rate']:>+14.1f}%")
print(f"{'平均回報':<15} {result_42['avg_return']:>14.2f}% {result_90['avg_return']:>14.2f}% {result_42['avg_return']-result_90['avg_return']:>+14.2f}%")

if result_42['annual_return'] > result_90['annual_return']:
    diff = result_42['annual_return'] - result_90['annual_return']
    print(f"\n✅ 42天優於90天，年化多出 {diff:.1f}%")
else:
    diff = result_90['annual_return'] - result_42['annual_return']
    print(f"\n❌ 90天優於42天，年化多出 {diff:.1f}%")
